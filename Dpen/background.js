// Copyright (c) 2015 Hao-Cher Hong. All rights reserved.
...
function getDcardUrl() {
  return "https://www.dcard.tw/";
}
...
function updateIcon() {
  if (!localStorage.hasOwnProperty('unreadCount')) {
    chrome.browserAction.setIcon({path:"images/icon_logged_out.png"});
...
function startRequest(params) {
  ...
  getAPI("member/status",
    function(response) {
      if(response.isLogin) {

        var unreadCount = (response.has_dcard && response.dcard) ? 1 : 0;

        unreadCount += response.newsUnread;

        getAPI("member/new_notification", function(response) {
          unreadCount += response.notification.filter(function(item) {
...
var filters = {
  ...
  url: [{urlContains: getDcardUrl().replace(/^https?\:\/\//, '')}]
};
